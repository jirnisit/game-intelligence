  ...
  props => {
    ...
    const values = computed(() =>
      props.entries.filter(
        (e, i, all) =>
          i === 0 || all[i - 1].value !== e.value || all[i - 1].unit !== e.unit || all[i - 1].per_stack !== e.per_stack,
      ),
    )
      ...
      >
        {values.value.map(e => (
          ...
          >
            ...
            {stats[e.stat_code] || ''}
            {' '}
            {e.value === null ? formatValue(e) : `+${formatValue(e)}`}
          </div>
        ))}
        {recipients.value.map((e, i) => (
          <div
            key={e.target + ':' + e.awakening_from}
            t-data='effect-recipient'
            class='mt-1 text-label-s'
          >
            {i > 0 ? prefix(e.awakening_from) : ''}
            {target(e.target)}
          </div>
        ))}
        {conditionLabel(props.entries[0].condition)
          ? (
              <div
