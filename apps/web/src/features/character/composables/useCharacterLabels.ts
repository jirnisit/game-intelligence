...
export function useCharacterLabels() {
  ...
  const effectLabel = (type: string) =>
    ({
      ...
      break_damage_taken_increase: t('ศัตรูรับดาเมจเกจ Break เพิ่ม', 'Enemy Break damage taken'),
  ...
  const formatValue = (e: { value: number | string | null, unit: string | null, per_stack: boolean }) =>
    e.value == null
      ? '—'
  ...
  const conditionLabel = (c: Record<string, unknown> = {}) => {
    ...
    const names: Record<string, string> = {
      ...
      sun: 'Sun',
      moon: 'Moon',
      dragons_wrath: 'Dragon’s Wrath',
      rookies_morale: 'Rookie’s Morale',
      darkness: 'Darkness',
      devotion: 'Devotion',
    }
    ...
    if (Array.isArray(c.requires_statuses)) {
      ...
      if (statuses.length)
        parts.push(
          (c.required_status_target === 'enemy' ? t('ศัตรูต้องมี ', 'Enemy requires ') : t('ต้องมี ', 'Requires '))
          + statuses.join(' + '),
        )
    }
    ...
    if (name(c.per_resource_consumed))
      parts.push(t('ต่อ stack ที่ใช้ของ ', 'Per consumed stack of ') + name(c.per_resource_consumed))
    ...
    if (c.gauge_basis === 'consumed' || c.gauge_basis === 'held')
      parts.push(
        c.gauge_basis === 'consumed'
          ? t('อิงเกจที่ใช้ไป', 'Based on consumed gauge')
          : t('อิงเกจที่มี', 'Based on held gauge'),
      )
    if (c.consume_entire_gauge) parts.push(t('ใช้เกจทั้งหมด', 'Consumes the entire gauge'))
    if (typeof c.per_gauge_percent_consumed === 'number')
      parts.push(t('ต่อเกจที่ใช้ทุก ', 'Per consumed ') + c.per_gauge_percent_consumed + t('%', '% gauge'))
    if (c.excludes === 'special_attacks') parts.push(t('ยกเว้นการโจมตีพิเศษ', 'Excludes special attacks'))
    if (Array.isArray(c.on_hits) && c.on_hits.every(v => typeof v === 'number'))
      parts.push(t('โจมตีครั้งที่ ', 'Hit ') + c.on_hits.join(', '))
    const events: Record<string, string> = {
      hit: t('เมื่อโจมตีโดน', 'On hit'),
      each_hit: t('ทุก hit', 'Each hit'),
      trigger_elemental_reaction: t('เมื่อเกิด Reaction', 'When a Reaction occurs'),
      use_in_sync_with_enemy_attack: t('ใช้ตรงกับจังหวะโจมตีของศัตรู', 'Use in sync with the enemy’s attack'),
      use_ultimate: t('เมื่อใช้อัลติเมต', 'On Ultimate use'),
    }
