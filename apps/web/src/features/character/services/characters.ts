...
export async function getCharacter(game: string, id: string, awakening: number, signal?: AbortSignal) {
  const detail = await getJson<Detail>(`/api/characters/${encodeURIComponent(id)}?awakening=${awakening}`, signal)
