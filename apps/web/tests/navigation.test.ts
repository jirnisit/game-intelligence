...
vi.mock("../src/core/config/games", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("../src/core/config/games")>();
  return {
    ...actual,
    games: [
      ...actual.games,
      { ...actual.games[0], id: "second-game", name: { en: "Second Game" } },
    ],
  };
});
...
async function click(selector: string) {
  ...
  if (href)
    await vi.waitFor(() => expect(router.currentRoute.value.path).toBe(href));
}
...
describe("Kab Game navigation and TSX", () => {
  it("shows the frontend game catalogue without fetching data, then filters within the game", async () => {
    ...
    expect(
      root
        .querySelector<HTMLImageElement>('[t-data="game-art"] img')!
        .getAttribute("src"),
    ).toBe("/limit-zero-breakers-logo.png");
    await click('[t-data="game-card"]');
    await settle();
    expect(router.currentRoute.value.path).toBe(
      "/game/limit-zero-breakers/characters",
    );
  ...
  it("supports direct team URLs, member links and a persistent language switch", async () => {
    ...
    expect(requests).toContain(
      "/api/teams?game=limit-zero-breakers&limit=24&offset=0",
    );
    expect(root.querySelectorAll('[t-data="team-card"]')).toHaveLength(1);
    await click('[t-data="team-card"] h2 a');
    await settle();
    expect(root.querySelector('[t-data="team-description"]')!.textContent).toBe(
      "1. Prepare\n2. Switch",
    );
...
it("reads game params when switching contexts and rejects unknown or mismatched games", async () => {
  ...
  expect(
    root.querySelector('[t-data="character-card"]')!.getAttribute("href"),
  ).toBe("/game/second-game/characters/mei");
  expect(
    root
      .querySelector('[t-data="section-nav"] a:last-child')!
      .getAttribute("href"),
  ).toBe("/game/second-game/teams");
