...
it("maps every semantic role to primitives and maintains readable text contrast in both themes", async () => {
  ...
  const values = Object.fromEntries(
    [...palette.matchAll(/--palette-([\w-]+):\s*#([\da-f]{6})/g)].map((m) => [
      m[1],
      m[2],
    ]),
  );
  ...
  const roles = [...colors.matchAll(/--color-([\w-]+):\s*var\(--sys-/g)].map(
    (m) => m[1],
  );
  expect(roles).toHaveLength(26);
  for (const theme of ["light", "dark"]) {
    const block = colors.split(`:root[data-theme="${theme}"]`)[1].split("}")[0];
    const mapping = Object.fromEntries(
      [...block.matchAll(/--sys-([\w-]+):\s*var\(--palette-([\w-]+)\)/g)].map(
        (m) => [m[1], m[2]],
      ),
    );
    for (const role of roles)
      expect(values[mapping[role]], `${theme}: ${role}`).toBeDefined();
    const pairs = ["primary", "secondary", "tertiary", "error"].flatMap(
      (role) => [
        [role, `on-${role}`],
    ...
    for (const [background, foreground] of pairs) {
      const a = luminance(values[mapping[background]]),
        b = luminance(values[mapping[foreground]]);
      expect(
